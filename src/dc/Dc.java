...
public class Dc {
    ...
    */
    ...
    private static void ContourEdgeProc(OctreeNode[] node, int dir, List<Integer> indexBuffer) {
        ...
        List<Vec3i> chunks = new ArrayList<>();
        for (int i = 0; i < 4; i++) {
    ...
    private static void ContourFaceProc(OctreeNode[] node, int dir, List<Integer> indexBuffer) {
        ...
        // bit of a hack but it works: prevent overlapping seams by only
        // processing edges that stradle multiple chunks
        if (ChunkMinForPosition(node[0].min).equals(ChunkMinForPosition(node[1].min))) {
            return;
        }
    ...
    static void ContourEdgeProc(OctreeNode[] node, int dir, List<Integer> buffer) {
        if (node[0] == null || node[1] == null || node[2] == null || node[3] == null) {
            return;
        }
    ...
    static void ContourFaceProc(OctreeNode[] node, int dir, List<Integer> buffer) {
        if (node[0] == null || node[1] == null) {
            return;
        }

        // bit of a hack but it works: prevent overlapping seams by only processing edges that stradle multiple chunks
        if (chunkMinForPosition(node[0]).equals(chunkMinForPosition(node[1]))) {
            return;
        }
    ...
    public static void ContourCellProc(OctreeNode node, List<Integer> buffer) {
