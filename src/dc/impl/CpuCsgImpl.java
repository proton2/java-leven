...
public class CpuCsgImpl implements ICSGOperations{
    ...
    @Override
    public void ApplyCSGOperations(MeshGenerationContext meshGen, List<CSGOperationInfo> opInfo, Vec3i clipmapNodeMin,
    ...
    private void processCSG(MeshGenerationContext meshGen, List<CSGOperationInfo> opInfo, Vec3i clipmapNodeMin, int clipmapNodeSize, GPUDensityField field) {
        ...
        int[] d_updatedMaterials = new int[fieldBufferSize];

        int numUpdatedPoints = CSG_HermiteIndices(fieldOffset, opInfo.size(), opInfo, sampleScale, field.materialsCpu,
                d_updatedIndices, d_updatedPoints, d_updatedMaterials);
        ...
        int[] d_compactUpdatedMaterials = new int[numUpdatedPoints];
        compactPoints(d_updatedIndices, d_updatedPoints, d_updatedMaterials,
                d_compactUpdatedPoints, d_compactUpdatedMaterials);
        UpdateFieldMaterials(d_compactUpdatedPoints, d_compactUpdatedMaterials, field.materialsCpu);

        int numGeneratedEdges = numUpdatedPoints * 6;
        int[] d_generatedEdgeIndices = new int [numGeneratedEdges];
        FindUpdatedEdges(d_compactUpdatedPoints, d_generatedEdgeIndices);

        int[] d_edgeIndicesValid = new int[numGeneratedEdges];
        int numCompactEdgeIndices = RemoveInvalidIndices(d_generatedEdgeIndices, d_edgeIndicesValid);

        int[] d_compactEdgeIndices = new int[numCompactEdgeIndices];
        CompactIndexArray(d_edgeIndicesValid, d_generatedEdgeIndices,
                d_compactEdgeIndices);

        int[] d_invalidatedEdges = Arrays.stream(d_compactEdgeIndices).distinct().toArray();
        int numInvalidatedEdges = d_invalidatedEdges.length;
        int[] d_edgeValidity = new int[numInvalidatedEdges];
        FilterValidEdges(d_invalidatedEdges, field.materialsCpu, d_edgeValidity);

        int[] d_createdEdges = new int[numInvalidatedEdges];
        CompactIndexArray(d_edgeValidity, d_invalidatedEdges,
                 d_createdEdges);

        if (numInvalidatedEdges > 0 && field.numEdges > 0) {
            int[] d_fieldEdgeValidity = new int[field.numEdges];
            int numPrunedEdges = PruneFieldEdges(field.edgeIndicesCpu, d_invalidatedEdges, numInvalidatedEdges,
                    d_fieldEdgeValidity);
            if (numPrunedEdges > 0) {
                int[] d_prunedEdgeIndices = new int[numPrunedEdges];
                Vec4f[] d_prunedNormals = new Vec4f[numPrunedEdges];
                CompactFieldEdges(d_fieldEdgeValidity, field.edgeIndicesCpu, field.normalsCpu, d_prunedEdgeIndices, d_prunedNormals);
        ...
        if (numCreatedEdges > 0) {
            ...
            FindEdgeIntersectionInfo(fieldOffset, opInfo.size(), opInfo, sampleScale, d_createdEdges, d_createdNormals);
            if (field.numEdges > 0) {
    ...
    private Map<Integer, Integer> compactPoints(int[] updated_indices,
                                                Vec4i[] points, int[] operationMaterials,
                                                Vec4i[] compact_points, int[] compactOpMaterials) {
        int current = 0;
        Map<Integer, Integer> edgeIndicatesMap = new HashMap<>(compactOpMaterials.length);
        for (int index = 0; index < updated_indices.length; index++) {
            if (updated_indices[index]==1) {
                //edgeIndicatesMap.put(edges[index], current);
                compact_points[current] = points[index];
                compactOpMaterials[current] = operationMaterials[index];
                ++current;
            }
        }
    ...
    private void UpdateFieldMaterials(Vec4i[] fieldPositions, int [] operationMaterials, int[] fieldMaterials) {
        for(int id=0; id<fieldPositions.length; id++) {
	        Vec4i fieldPos = fieldPositions[id];
	        int fieldIndex = field_index(fieldPos);
            fieldMaterials[fieldIndex] = operationMaterials[id];
        }
    }

    private void FindUpdatedEdges(Vec4i[] updatedHermiteIndices,
    ...
    private int RemoveInvalidIndices(int[] generatedEdgeIndices,
            int[] edgeValid)
    {
        int size=0;
        for(int id=0; id<generatedEdgeIndices.length; id++) {
            edgeValid[id] = generatedEdgeIndices[id] != -1 ? 1 : 0;
            if(generatedEdgeIndices[id] != -1){
                size++;
            }
        }
        return size;
    ...
    private void CompactIndexArray(int[] edgeValid, int[] edgeIndices,
            int[] compactIndices) {
        int current = 0;
	    for(int index=0; index<edgeValid.length; index++)
        if (edgeValid[index]==1) {
            compactIndices[current] = edgeIndices[index];
            ++current;
        }
    }

    private int PruneFieldEdges(int[] fieldEdgeIndices, int[] invalidatedEdgeIndices, int numInvalidatedEdges,
            int[] fieldEdgeValidity)
    {
        int count = 0;
        for (int id = 0; id<fieldEdgeIndices.length; id++) {
            int edgeIndex = fieldEdgeIndices[id];
            int invalid = 0;
            for (int i = 0; i < numInvalidatedEdges; i++) {
                invalid |= (invalidatedEdgeIndices[i] == edgeIndex) ? 1 : 0;
            }
            fieldEdgeValidity[id] = invalid == 1 ? 0 : 1;
            if(fieldEdgeValidity[id]==1){
                count++;
            }
        }
        return count;
    ...
    private void CompactFieldEdges(int[] edgeValid, int[] edgeIndices,
            Vec4f[] edgeNormals, int[] compactIndices, Vec4f[] compactNormals)
    {
        int cid = 0;
        for (int index = 0; index<edgeValid.length; index++) {
            if (edgeValid[index]==1) {
    ...
    private void FindEdgeIntersectionInfo(Vec4i offset, int numOperations, List<CSGOperationInfo> operations, int sampleScale, int[] compactEdges,
                                          Vec4f[] normals) {
        for(int index = 0; index<compactEdges.length; index++) {
	        ...
	        float t = BrushZeroCrossing(p0, p1, numOperations, operations);
	        Vec4f p = VoxelHelperUtils.mix(p0, p1, t);

	        Vec3f n = BrushNormal(p, numOperations, operations);
            normals[index] = new Vec4f(n, t);
        }
    }

    private void FilterValidEdges(
            int[] generatedHermiteEdgeIndices,
            int[] materials,
            int[] edgeValid
    ) {
        for (int id = 0; id < generatedHermiteEdgeIndices.length; id++) {
            int generatedEdgeIndex = generatedHermiteEdgeIndices[id];
            ...
            edgeValid[id] = signChange==1 && generatedEdgeIndex != -1 ? 1 : 0;
        }
    }
    ...
    private float BrushZeroCrossing(Vec4f p0, Vec4f p1, int numOperations, List<CSGOperationInfo> operations) {
        float minDensity = Float.MAX_VALUE;
        float crossing = 0.f;
        for (float t = 0.f; t <= 1.f; t += (1.f/16.f)) {
		    Vec4f p = VoxelHelperUtils.mix(p0, p1, t);
            for (int i = 0; i < numOperations; i++) {
			    CSGOperationInfo op = operations.get(i);
    ...
    private int BrushMaterial(Vec4f world_pos, int numOperations, List<CSGOperationInfo> operations, int material) {
        int m = material;
        for (int i = 0; i < numOperations; i++) {
		    CSGOperationInfo op = operations.get(i);
		    int[] operationMaterial = {op.getMaterial(), 201,};
    ...
    private Vec3f BrushNormal(Vec4f world_pos, int numOperations, List<CSGOperationInfo> operations) {
    ...
    private int CSG_HermiteIndices(Vec4i worldspaceOffset, int numOperations, List<CSGOperationInfo> operations, int sampleScale,
                                    int[] field_materials,
                                    int[] updated_indices, Vec4i[] updated_positions, int[] updatedMaterials) {
        int size = 0;
